...
// 输入
const layoutTree: PageLayoutTree = {
  "root": [
    {
      "id": "Container_1",
      "type": "container"
...
function buildComponentTree(layoutTree: PageLayoutTree, nodeId: string): LayoutConfig {
  ...
  const children = node.map(child => {
    const childNode = buildComponentTree(layoutTree, child.id);
    return childNode;
  });
...
const renderableComponentTree = buildComponentTree(layoutTree, 'root');
